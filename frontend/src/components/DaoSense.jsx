...
export default function DaoSense({lang="ua"}){
 return <section id="dao" className="dao-section"><div className="max-w-6xl mx-auto px-5">
  ...
  <div className="tea-pour reveal"><div className="tea-steam"><i/><i/><i/></div><div className="tea-pot"><span className="tea-lid"/><span className="tea-body"/><span className="tea-handle"/><span className="tea-spout"/></div><div className="tea-copy"><span>下一泡 · NEXT POUR</span><strong>{lang==="ua"?"Додаємо ще води для наступного проливу":"Add more water for the next pour"}</strong><p>{lang==="ua"?"Цикл не закінчується. Нові дані повертають нас до Води.":"The cycle does not end. New data returns to Water."}</p><a href="#stages"><ArrowDown size={15}/>{lang==="ua"?"Далі — етапи":"Next — stages"}</a></div></div>
 </div></section>;
}
