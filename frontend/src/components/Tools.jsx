...
export default function Tools({title,lang="ua"}){
 const [selected,setSelected]=useState(null),isUa=lang==="ua";
 return <section id="tools" className="tools-r4 relative px-5"><div className="max-w-6xl mx-auto">
  ...
  <div className="tool-element-stack">{ORDER.map(id=>{const e=ELEMENT_SYSTEM[id],t=e[lang]||e.ua,Icon=ICONS[id],tools=toolsForElement(id);return <section className="tool-element-group panel reveal" style={{"--element-color":e.color}} key={id}><header className="tool-element-head"><span className="tool-element-icon"><Icon size={19}/></span><div><span>{t.name} · {t.phase}</span><strong>{t.question} → {t.outcome}</strong></div><em>{tools.length} {isUa?"інструментів":"tools"}</em></header><p className="tool-element-intro">{t.summary}</p><div className="tool-compact-grid">{tools.map(tool=>{const x=tool[lang]||tool.ua;return <button type="button" key={tool.id} onClick={()=>setSelected(tool)} className="tool-compact-card"><span>{x.name}</span><small>{x.solves}</small><ArrowUpRight size={13}/></button>})}</div></section>})}</div>
